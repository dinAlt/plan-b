import { ITask, ITrigger } from "./types";
import { intervalToMilliseconds } from "./util";

function validateActions(task: ITask): void {
...
function validateTriggers(task: ITask): void {
    task
        .triggers
        .forEach(t => {
            if (t.on !== "init") {
...
function validateTriggerPlan(trigger: ITrigger, task: ITask): void {
    if (trigger.plan) {
        if (trigger.plan === "once") {
            return;
        }
        if (typeof trigger.plan === "string") {
            intervalToMilliseconds(trigger.plan);
            return;
        }
        if (trigger.plan.length &&
            (trigger.plan.length === 2) && (typeof trigger.plan[0] === "number")) {
            intervalToMilliseconds(trigger.plan[1]);
            return;
        }
